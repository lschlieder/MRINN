...
#include "Losses/sparse_softmax_crossentropy.h"
...
int main() {
	...
	std::ifstream ifsload1("saved_net_images_knees_LOAD_TEST.dat");
...
//adam test below:
/*
    MRINN::Optimizers::Adam * adam_test = new MRINN::Optimizers::Adam((double)0.001,(double)0.9,(double)0.999);
    adam_test->set_size(10);
    std::vector<float> params(10,0.1);
    std::vector<float> grad(10,0.1);
    std::cout << "test" << std::endl;
    for( int j = 0; j < 20; j++){
    std::vector<float> upd = adam_test->get_updated_parameters(params,grad);
    for( int i=0; i < 10; i++){
        std::cout << upd[i] << " ";
    }
    std::cout << std::endl;
...
// convolution test below
	...
	vector<int> test_out(3,4); test_out[0] = 1;

    Convolution2DCudnn convo_test(test_inp, test_out,3,1, true);
    std::vector<float> params(19,0.01);
    params[18] = 0;
    //std::vector<float> params(4,0.01);
    //params[3] = 0;
    convo_test.set_var_parameters(params);
   // Convolution2DLayer conv_comp(test_inp,test_out, 3, 1, Convolution2DLayer::zero_padded);
    ConvolutionalLayer conv_comp(test_inp,test_out, 3, 1, dnnBorderZeros, false);
    
    Losses::Sparse_Softmax_Crossentropy test_entrop = Losses::Sparse_Softmax_Crossentropy();
    Losses::MSE test_mse = Losses::MSE();
    
    //convo_test.set_cudnn_handle(cudnn);;
    VectorNd<float>inpt(2,2,4*4*2);
	inpt.fill(1.0);
    ...
    VectorNd<float> wanted_output(2,2,4*4);
    wanted_output.fill(0.0);
    wanted_output(2,0,0) = 1.0;
    wanted_output(2,1,0) = 1.0;
    std::cout << "wanted_output " << std::endl;
    wanted_output.print();
    std::cin.ignore();
    double loss = test_mse.get_loss(wanted_output,out_test2);
	...
	//std::cout << "error inp";
	//errort.print();
	VectorNd<float> erroroutt = convo_test.calc_error(out_test2);
    ...
    VectorNd<float> gradient = convo_test.get_parameter_gradient(out_test2);
    ...
    //vector<float> params = convo_test.get_var_parameters();
    //for (int i =0; i< params.size();i++){
    //    params[i] = params[i] - gradient.get_data()[i];
    //}
    //convo_test.set_var_parameters(params);
    //vector<float> new_params = convo_test.get_var_parameters();
    //for (int i =0; i< new_params.size();i++){
    //    std::cout << new_params[i] << std::endl;
    //}
    cin.ignore();
...
/*
	...
	Convolution2DCudnn convo_test = Convolution2DCudnn(test_inp, test_out,3,2, true);
	//ConvolutionalLayer convo_nd_test = ConvolutionalLayer(test_inp, test_out, 3,2, dnnBorderZeros,true);
    Convolution2DLayer convo_nd_test = Convolution2DLayer(test_inp, test_out, 3,2,Convolution2DLayer::zero_padded);
...
/*
	vector<int> test_size(3,4); test_size[0] = 3;
	vector<int> test_size_out(3,2); test_size_out[0] = 3;
	MaxPoolingLayer pool = MaxPoolingLayer(test_size, test_size_out,2,2);
	VectorNd<float> testinp(2,2,3*16); testinp.fill(0.5); testinp(2,0,2) = 1.0; testinp(2,0,9) = 3.0; testinp(2,0,14) = 4; testinp(2,1,14) = 4;
	//std::cout <<" test" << std::endl;
	testinp.print();
	VectorNd<float> testout = pool.calc_output(testinp);
	testout.print();
    
    VectorNd<float> testerr(2,2,3*4); testerr.fill(0.0); testerr(2,0,2) = 1.0; testerr(2,1,2)= 1.0; testerr.fill(1.0);
    testerr.print();
    VectorNd<float> testerrout = pool.calc_error(testerr);
    testerrout.print();
    */
    ...
    std::vector<float> params(6,0.01);
    params[2]  = 0;
    params[5] = 0;
    test_dense.set_var_parameters(params );
    Losses::Sparse_Softmax_Crossentropy test_entrop = Losses::Sparse_Softmax_Crossentropy();
	VectorNd<float> test_input(2,10,2);
	test_input.fill(2.0);

	VectorNd<float> test_out = test_dense.calc_output(test_input);
	test_out.print();
    
    VectorNd<float> wanted_output(2,10,2);
    wanted_output.fill(1.0);
    double error = test_entrop.get_loss(wanted_output,test_out);
    test_out.print();

    
	VectorNd<float> error_test_out = test_dense.calc_error(test_out);
	VectorNd<float> gradient = test_dense.get_parameter_gradient(test_out);
	...
	vector<int> inp_size1(3,28); inp_size1[0] = 1; // 748
	vector<int> out_size1(3,28); out_size1[0] = 10;
	vector<int> out_size2(3,14); out_size2[0] = 10;
	vector<int> out_size3(3,14); out_size3[0] = 20;
	vector<int> out_size4(3,7); out_size4[0] = 20;
	vector<int> out_size5(3,7); out_size5[0] = 30;
    
	vector<int> out_size6(3,3); out_size6[0] = 2;

	/*vector<int> mnist_size1(3,28); mnist_size1[0] = 1;
	vector<int> mnist_size2(3,28); mnist_size2[0] = 10;
	vector<int> mnist_size3(3,14); mnist_size3[0] = 10;
	vector<int> mnist_size4(3,14); mnist_size4[0] = 20;
	vector<int> mnist_size5(3,7); mnist_size5[0] = 20;
	vector<int> mnist_size6(3,7); mnist_size6[0] = 30;
	vector<int> mnist_size7(3,3); mnist_size7[0] = 30;*/
    
    vector<int> mnist_size1(3,32); mnist_size1[0] = 3;
	vector<int> mnist_size2(3,32); mnist_size2[0] = 10;
	vector<int> mnist_size3(3,16); mnist_size3[0] = 10;
	vector<int> mnist_size4(3,16); mnist_size4[0] = 20;
	vector<int> mnist_size5(3,8); mnist_size5[0] = 20;
	vector<int> mnist_size6(3,8); mnist_size6[0] = 30;
	vector<int> mnist_size7(3,3); mnist_size7[0] = 30;
   ...
   /*net->add_layer(convo_first);
    ...
    //cudnnHandle_t cudnn_context;
    //cudnnCreate(&cudnn_context);
	//net->add_layer( new ConvolutionalLayer(mnist_size1, mnist_size2, 5,10, dnnBorderZeros,true));
    net->add_layer( new Convolution2DCudnn(mnist_size1, mnist_size2, 5,10,true));
   
	net->add_layer( new ReluLayer(mnist_size2, mnist_size2));
	net->add_layer( new MaxPoolingLayer( mnist_size2, mnist_size3, 2,2));

	//net->add_layer( new ConvolutionalLayer( mnist_size3, mnist_size4, 5,20, dnnBorderZeros,true));
    net->add_layer( new Convolution2DCudnn(mnist_size3, mnist_size4, 5,20,true));
	net->add_layer( new ReluLayer( mnist_size4, mnist_size4));
	net->add_layer( new MaxPoolingLayer( mnist_size4, mnist_size5, 2,2));
	//net->add_layer( new ConvolutionalLayer( mnist_size5, mnist_size6, 5,30, dnnBorderZeros,true));
    net->add_layer( new Convolution2DCudnn(mnist_size5, mnist_size6, 5,30,true));
	net->add_layer( new ReluLayer( mnist_size6, mnist_size6));
   ...
   // net->add_layer(new DenseLayer(std::vector<int>(1,100), std::vector<int>(1,100), false));
	//net->add_layer(new DenseLayer(std::vector<int>(1,100), std::vector<int>(1,100), false));
	//net->add_layer(new DenseLayer(mnist_size6, std::vector<int>(1,100), true));
	//net->add_layer(new Convolution2DCudnn(mnist_size1, mnist_size2,3,1,true));
    //net->add_layer(new ReluLayer(out_size1, out_size1));
    //net->add_layer(new ConvolutionalLayer(mnist_size1, mnist_size2, 3,1,dnnBorderZeros,true));
    
   // net->add_layer(new ReluLayer( mnist_size2, mnist_size2));
    //net->add_layer(new ReluLayer( mnist_size2, mnist_size2));
    //net->add_layer(new DenseLayer(mnist_size1, std::vector<int>(1,100), true));
    net->add_layer(new  DenseLayer(mnist_size6, std::vector<int>(1,100), true));
    net->add_layer(new ReluLayer( std::vector<int>(1,100), std::vector<int>(1,100)));
    net->add_layer(new DenseLayer(std::vector<int>(1,100), std::vector<int>(1,10), true));
	...
	//noisy_mnist = Datasets::MNIST::load_noisy_mnist_train(noisy_mnist,50000,0.1);
	//noisy_mnist_test= Datasets::MNIST::load_noisy_mnist_test(noisy_mnist_test,10000,0.1);

    vector<Data*> * cifar10_train = new vector<Data*>(0);// = Datasets::CIFAR10::load_noisy_CIFAR10_train(noisy_cifar10, 40000, 0.3);
	vector<Data*> * cifar10_test =  new vector<Data*>(0);// = Datasets::CIFAR10::load_CIFAR10_test(noisy_cifar10_test,10000,0.3);
	//cifar10_train = Datasets::CIFAR10::load_CIFAR10_train(cifar10_train, 50000);
    Datasets::CIFAR10::load_CIFAR10_train(cifar10_train, 50000);
    Datasets::CIFAR10::load_CIFAR10_test(cifar10_test,10000);
    
    /*ofstream cifartestfile = ofstream("images_cifar10/testfile.txt");
    for( int i =0; i < 100;i++){
		VectorNd<float> inp(2,1,32*32*3);

		std::copy((*cifar10_test)[i]->inp.begin(), (*cifar10_test)[i]->inp.end(), &inp.get_data()[0]);

        
        CImg<float> cifar10img(&inp.get_data()[0],32,32,1,3);
        cifar10img = cifar10img*255;
        cifar10img.save("images_cifar10/testimg_.bmp",i);
        for( int j = 0; j < 10;j++){
        cifartestfile << (*cifar10_test)[i]->out[j]<< " " ;
        }
        
        cifartestfile <<"  testimg_" << i<<  std::endl;
	}
	cifartestfile.close();*/
    
	//vector<Data> noisy_knees;
	//vector<Data> noisy_knees_test;
	//noisy_knees = Datasets::FullySampledKnees::load_noisy_knees_train_single_sigma(noisy_knees,3000,0.13);
    ...
    //vector<Data*> * mnist = new vector<Data*>(0);
	//vector<Data*> * mnist_test = new vector<Data*> (0);
    //std::cout << sizeof(*mnist) << " " << mnist->size()<< " "  ;
	//Datasets::MNIST::load_mnist_train(mnist,50000);
    //std::cout << sizeof(*mnist)  <<" "<< mnist->size() <<  std::endl;
	//Datasets::MNIST::load_mnist_test(mnist_test,10000);
    //std::cout << "finished loaded MNIST data!" << std::endl;
...
*/
	...
	net->set_data(*cifar10_train);
    net->set_validation_data(*cifar10_test);
	std::cout << "finished setting MNIST data!" << std::endl;
	training_opts opts;
	opts.batch_size = 100;
	opts.epochs = 20;
    opts.log_loss = true;
    opts.log_filename = "Logfiles/logfile_CIFAR10_gpu_mse_adagrad.txt";
    opts.validation_log_filename = "Logfiles/validation_logfile_CIFAR10_gpu_mse_adagrad.txt";
    opts.use_validation_dataset= true;
	Optimizers::SGD *sgd = new Optimizers::SGD(0.004,0.9,false);
	//Optimizers::AdaDelta optim = Optimizers::AdaDelta(0.01, 0.95f);
	Optimizers::AdaGrad *adagrad = new Optimizers::AdaGrad(0.01f);
	Optimizers::Adam *adam = new Optimizers::Adam(0.001,0.9,0.999);
    
	Optimizers::AdaMax *adamax = new Optimizers::AdaMax(0.002,0.9,0.999);
	...
	//Losses::CrossEntropy *crossent = new Losses::CrossEntropy();
    //Losses::BinaryCrossentropy* bin_crossent = new Losses::BinaryCrossentropy();
    Losses::Sparse_Softmax_Crossentropy *sparse_softm_cross = new Losses::Sparse_Softmax_Crossentropy();
    //Losses::BinaryCrossentropy
    //opts.loss = bin_crossent;
    //opts.loss = crossent;
	opts.loss = mse;
    //opts.loss = sparse_softm_cross;
    ...
    //delete net;
	//std::ofstream ofs("saved_net_images_knees_LOAD_TEST.dat");
	...
	for( int i =0; i < 100;i++){
        ...
        //std::cout << "test" << std::endl;
		VectorNd<float> inp(2,1,32*32*3);
        //VectorNd<float> actual_noise_compl(2,1,32*32*3);
        //std::cout << "test" << std::endl;
		std::copy(cifar10_test[i].inp.begin(), cifar10_test[i].inp.end(), &inp.get_data()[0]);
        //std::copy(noisy_knees[i].out.begin(), noisy_knees[i].out.end(), actual_noise_compl.get_data().begin());
        //std::cout << "test" << std::endl;
		...
		//VectorNd<float> output = net->calc_output(inp);
        //std::cout << "knees_inp: " << noisy_knees[i].inp[0] << " " << noisy_knees[i].inp[1] << std::endl;
        //std::cout << "copied input:" << inp.get_data()[0] << " " << inp.get_data()[1] << std::endl;
        VectorNd<float> out(2,1,32*32*3);
        ...
        for(int j =0; j < 2500;j++){
                ...
                double real_inp = noisy_knees[i].inp[j];
                double imag_inp = noisy_knees[i].inp[j+2500];
                ...
                double real_denoised = noisy_knees[i].inp[j] - denoiser_output.get_data()[j];
                double imag_denoised = noisy_knees[i].inp[j+2500] - denoiser_output.get_data()[j+2500];
                ...
                double real_perfect_image = noisy_knees[i].inp[j] - actual_noise_compl.get_data()[j];
                double imag_perfect_image = noisy_knees[i].inp[j+2500] - actual_noise_compl.get_data()[j+2500];
		...
		CImg<float> denoiser_output_img(&out.get_data()[0],50,50,);
        ...
        CImg<float> cifar10img(&inp.get_data()[0],32,32,3);
		...
		//noisy_img.save("images_knees_cuda/noisy_img_2_.bmp",i);
		//noise.save("images_knees_cuda/noise_2_.bmp",i);
		//denoiser_output_img.save("images_knees_cuda/denoiser_output_2_.bmp",i);
		//denoised_img.save("images_knees_cuda/denoised_image_2_.bmp",i);
		//denoised_img_real.save("images_knees_cuda/denoised_img_real_2_.bmp",i);
