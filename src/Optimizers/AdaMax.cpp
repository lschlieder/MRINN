...
AdaMax::AdaMax(float learning_rate = 0.002, float momentum_one = 0.9, float momentum_two = 0.999) {
...
vector<float> & AdaMax::get_updated_parameters(vector<float> & parameters, vector<float> & gradient){
	#pragma omp parallel for
	for( unsigned int i=0; i < gradient.size(); i++){
		//gradient_squared[i] = gradient[i]*gradient[i];
		//gradient[i] = gradient[i];
		momentum_vec_one[i] = momentum_one * momentum_vec_one[i];
		momentum_vec_two[i] = std::max(momentum_two * momentum_vec_two[i], abs(gradient[i]) );
	}
	...
	float alpha_t = learning_rate /(1-momentum_one_power_t);

	#pragma omp parallel for
	for ( unsigned int i = 0; i < gradient.size();i++){
		gradient[i] = alpha_t * momentum_vec_one[i]/momentum_vec_two[i];
